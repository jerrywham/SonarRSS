 ...
 *   txtj to justify
 *   User replacement rules can't be used as regexes anymore (incompatible with css file's commands system)
 *   a new command in the css allows to change auto_css configuration
 *    
 *   /*INITIALISATION
...
*/ 
    ...
    $auto_css['auto_add_css_links']=true; // Auto adds the link to css files 
     ...
     */ 
    $auto_css['path_to_css_file']='design/'; 
...
$auto_css_replace=array(
    //text
        ...
        "txtj(!important)?"=>"text-align: justify$1;",
    ...
    //divs
        "boxshadow ([0-9]+) ([0-9]+) (\#?[A-Za-z0-9]+)(!important)?"=>"-moz-box-shadow: 0 $1px $2px $3$4; -webkit-box-shadow: 0 $1px $2px $3$4; box-shadow: 0 $1px $2px $3$4; -ms-filter: \"progid:DXImageTransform.Microsoft.Shadow(Strength=$2, Direction=135, Color='$3')\"; filter: progid:DXImageTransform.Microsoft.Shadow(Strength=$2, Direction=135, Color='$3'); $pie ",
        ...
        "insetshadow ([0-9]+) ([0-9]+)(\#?[A-Za-z0-9]+)(!important)?"=>"-moz-box-shadow: inset 0 $1px $2px $3$4; -webkit-box-shadow: inset 0 $1px $2px $3$4; box-shadow: inset 0 $1px $2px $3$4; $pie ",
        "insetshadow ([0-9]+) ([0-9]+)(!important)?"=>"-moz-box-shadow: inset 0 $1px $2px #000000$3; -webkit-box-shadow: inset 0 $1px $2px #000000$3; box-shadow: inset 0 $1px $2px #000000$3; $pie ","opacity ([0-9]+)(!important)?"=>"opacity: .$1$2;filter: alpha(opacity=$1)$2;-ms-filter: \"alpha(opacity=$1)\"$2; -khtml-opacity: .$1$2; -moz-opacity: .$1$2; ",
    ...
    //pseudo classes
        "(?<=:)(?:pair|even)"=>"nth-child(even)",
        "(?<=:)(?:\impair|odd)"=>"nth-child(odd)",
        "(?<=:)fot"=>"first-of-type",
        "(?<=:)lot"=>"last-of-type",
        "(?<=:)oot"=>"only-of-type",
        "(?<=:)fch"=>"first-child",
...
function regenere_css($css_file,$css){
    ...
    // process configuration overwriting
    $reg_config='#(config)\(([a-zA-Z_]+),"([a-zA-Z0-9_ ]+)"\)#';//config(key,val)
    $nb= preg_match_all($reg_config,$variables,$vars);
   
    if ($nb>0){
        for ($i=0;$i<count($vars[1]);$i++){
            if (strtolower($vars[3][$i])=="true"){$vars[3][$i]==true;}
            if (strtolower($vars[3][$i])=="false"){$vars[3][$i]==false;}
            $auto_css[$vars[2][$i]]=$vars[3][$i];
        }
    }
    ...
    if ($auto_css['allow_auto_css_regex_replacement']==true){$css=auto_parse($css);}
